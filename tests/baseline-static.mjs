...
const models = await readFile(new URL('../api/_lib/models.js', import.meta.url), 'utf8');
...
const checks = [
  ...
  ['model routing separates grounded, utility, and TTS workloads', /DEFAULT_GROUNDED_MODEL\s*=\s*'gemini-2\.5-flash'/.test(models) && /DEFAULT_UTILITY_MODEL\s*=\s*'gemini-3\.5-flash'/.test(models) && /DEFAULT_TTS_MODEL\s*=\s*'gemini-2\.5-flash-preview-tts'/.test(models)],
  ['thinking configuration is selected by model generation', /thinkingLevel/.test(models) && /thinkingBudget/.test(models) && [chat, moderation, translation].every(source => /textGenerationConfig/.test(source) && !/temperature\s*:/.test(source))],
  ['TTS uses centralized audio-model selection', /ttsModel/.test(tts) && !/gemini-3\.5-flash/.test(tts)],
];
